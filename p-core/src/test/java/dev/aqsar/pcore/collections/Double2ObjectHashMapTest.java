...
class Double2ObjectHashMapTest {

    private Double2ObjectHashMap<String> map;

    @BeforeEach
    void setUp() {
        map = Double2ObjectHashMap.<String>builder().build();
    }
    ...
    @Test
    void testPutAll() {
        var source = new java.util.HashMap<Double, String>();
        source.put(1d, "one");
        source.put(2d, "two");
        source.put(3d, "three");
    ...
    void testSpecial() {
        ...
        map.put(Double.NaN, "2");
        assertEquals(1, map.size());
        assertEquals("2", map.get(Double.NaN));

        map.clear();

        map.put(Double.NEGATIVE_INFINITY, "1");
        ...
        map.put(Double.POSITIVE_INFINITY, "2");
        assertEquals(1, map.size());
        assertEquals("2", map.get(Double.POSITIVE_INFINITY));
    ...
    void testResizeGrowth() {
        map = Double2ObjectHashMap.<String>builder().initialCapacity(4).build();
    ...
    void testIteratorExhaustion() {
        var iters = new Double2ObjectHashMap.DoubleObjectHashMapIterator[8];
        for (int i = 0; i < 8; i++) {
    ...
    void testIteratorTraversal() {
        ...
        try (var iter = map.borrowIterator()) {
            ...
            assertTrue(keys.containsAll(Set.of(1d, 2d, 3d)));
    ...
    void testIteratorPeekAndNextKey() {
        ...
        try (var iter = map.borrowIterator()) {
            ...
            double firstKey = iter.peekNextKey();
    ...
    void testForEachEmpty() {
        map.forEachDoubleObject((k, v) -> fail("Should not be called on empty map"));
    }

    @Test
    void testBoxedPutAndGet() {
        map.put(Double.valueOf(1.1), "one");
        map.put(Double.valueOf(2.2), "two");

        assertEquals("one", map.get(Double.valueOf(1.1)));
        assertEquals("two", map.get(Double.valueOf(2.2)));
        assertNull(map.get(Double.valueOf(999)));
    }
    ...
    void testBoxedContains() {
        ...
        assertTrue(map.containsValue("one"));
        assertFalse(map.containsKey(Double.valueOf(2.2)));
    ...
    void testBoxedRemove() {
        ...
        assertEquals(2, map.size());
        assertEquals("one", map.remove(Double.valueOf(1.1)));
        assertNull(map.remove(Double.valueOf(999)));
        assertEquals(1, map.size());
    }
    ...
    void testBuilderInitialCapacity() {
        map = Double2ObjectHashMap.<String>builder().initialCapacity(128).build();
        for (int i = 0; i < 50; i++) {
            map.put(i, "value-" + i);
        }
    ...
    void testBuilderLoadFactor() {
        map = Double2ObjectHashMap.<String>builder().initialCapacity(16).loadFactor(0.5f).build();
        assertNotNull(map);
    }

    @Test
    void testBuilderDisableIteratorPool() {
        map = Double2ObjectHashMap.<String>builder().disableIteratorPool().build();
        assertEquals(0, map.availableIteratorCount());
        assertNull(map.borrowIterator());
    }
    ...
    void testKeySetView() {
        ...
        assertTrue(keySet.contains(1d));
        assertTrue(keySet.contains(2d));
        assertTrue(keySet.contains(3d));
        assertFalse(keySet.contains(4d));
    }
